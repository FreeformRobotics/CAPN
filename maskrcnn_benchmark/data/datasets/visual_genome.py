...
class VGDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, split, img_dir, roidb_file, dict_file, image_file, transforms=None,
        ...
        self.freqency_matrix = np.zeros([50, 150])
    ...
    def get_statistics(self):
        # print('dataloader, starting get_statics')
        # fg_matrix, bg_matrix, rel_class_num_lst, obj_class_num_lst, rel_counter = get_VG_statistics(self)

        fg_matrix, bg_matrix, rel_class_num_lst, obj_class_num_lst = get_VG_statistics(img_dir=self.img_dir, roidb_file=self.roidb_file, dict_file=self.dict_file,
                                                image_file=self.image_file, must_overlap=True)
        # print('fg_matrix.shape:', fg_matrix.shape)  # 151, 151, 51
        # print('bg_matrix.shape:', bg_matrix.shape)  # 151, 151, 51
        ...
        result = {
            ...
            'rel_class_num_lst': rel_class_num_lst,
            'obj_class_num_lst': obj_class_num_lst,
            'obj_classes': self.ind_to_classes,
            'rel_classes': self.ind_to_predicates,
            'att_classes': self.ind_to_attributes,
        }
        # print('fg_matrix.shape:', result['fg_matrix'])  # 151, 151, 51
        # print('pred_dist.shape:', result['pred_dist'])  # 151, 151, 51
        # print('rel_class_num_lst:', result['rel_class_num_lst'])
        #
        # print('obj_classes.shape:', len(result['obj_classes']))  # 151
        # print('rel_classes.shape:', len(result['rel_classes']))  # 51
        # print('att_classes.shape:', len(result['att_classes']))  # 201
    ...
    def get_groundtruth(self, index, evaluation=False, flip_img=False):
        ...
        # print('labels:', target.get_field("labels").shape, target.get_field("labels"))
        # print('attributes:', target.get_field("attributes").shape, target.get_field("attributes"))
        # print('relation:', target.get_field("relation").shape, target.get_field("relation"))
...
#     rel_class_num_lst = np.zeros(num_rel_classes)
#     obj_class_num_lst = np.zeros(num_obj_classes)

#     rel_counter = Counter()

#     # print('train_data.ind_to_classes:', train_data.ind_to_classes, num_obj_classes)
#     # print('train_data.ind_to_predicates:',  train_data.ind_to_predicates, num_obj_classes)
...
#         # print('gt_classes:', gt_classes.shape, gt_classes)

#         gt_relations = train_data.relationships[ex_ind].copy()
#         # print('gt_relations:', gt_relations.shape, gt_relations)

#         rel_results = gt_relations[:, 2]

#         for rel in rel_results:
#             rel_class_num_lst[rel] = rel_class_num_lst[rel] + 1

#         for obj in gt_classes:
#             obj_class_num_lst[obj] = obj_class_num_lst[obj] + 1
#         # print('obj_class_num_lst:', obj_class_num_lst)
...
#     return fg_matrix, bg_matrix, rel_class_num_lst, obj_class_num_lst, rel_counter

# def count_background_pairs(defined_relationships):
#     # Extract all unique objects
#     all_objects = set()
#     for rel in defined_relationships:
#         all_objects.add(rel[0])  # Subject
#         all_objects.add(rel[1])  # Object

#     # Create all possible pairs of distinct objects
#     all_possible_pairs = set(itertools.permutations(all_objects, 2))

#     # Exclude pairs where subject and object are the same
#     all_possible_pairs = set(pair for pair in all_possible_pairs if pair[0] != pair[1])

#     # Create a set of defined relationships (excluding relationship type)
#     defined_pairs = set((rel[0], rel[1]) for rel in defined_relationships)

#     # Find background pairs by excluding defined relationships
#     all_overlapping_pairs = all_possible_pairs - defined_pairs

#     # Return the number of background pairs
#     return len(all_overlapping_pairs)


def count_background_pairs(gt_relations):
    # Extract subjects and objects
    subjects = gt_relations[:, 0]
    objects = gt_relations[:, 1]

    # Create all possible pairs of distinct subjects and objects
    all_possible_pairs = set((s, o) for s in subjects for o in objects if s != o)

    # Create a set of defined relationships (excluding relationship type)
    defined_pairs = set((rel[0], rel[1]) for rel in gt_relations)

    # Find background pairs by excluding defined relationships
    background_relations = all_possible_pairs - defined_pairs

    # Return the number of background pairs
    return len(background_relations)
...
def get_VG_statistics(img_dir, roidb_file, dict_file, image_file, must_overlap=True):
    ...
    total_background_relationships = 0.0
    fg_matrix = np.zeros((num_obj_classes, num_obj_classes, num_rel_classes), dtype=np.int64)
    bg_matrix = np.zeros((num_obj_classes, num_obj_classes), dtype=np.int64)
    ...
    for ex_ind in tqdm(range(len(train_data))):
        ...
        gt_relations = train_data.relationships[ex_ind].copy()
        # print('gt_relations:', gt_relations.shape, gt_relations)
        background_relations = count_background_pairs(gt_relations)
        # print('background_relations:', background_relations)
        total_background_relationships += background_relations
        
        rel_results = gt_relations[:, 2]
        # print('rel_results:', rel_results)
        ...
        for (o1, o2) in o1o2_total:
            bg_matrix[o1, o2] += 1
        # print('bg_matrix:', bg_matrix.shape, bg_matrix)
    
    # total_background_relationships = np.sum(bg_matrix)
    # print('total_background_relationships:', total_background_relationships)
    # print('rel_counter:', rel_counter)

    return fg_matrix, bg_matrix, rel_class_num_lst, obj_class_num_lst
