...
def do_vg_evaluation(
    ...
    if "relations" in iou_types:
        ...
        eval_precision = SGPrecision(result_dict)
        eval_precision.register_container(mode)
        evaluator['eval_precision'] = eval_precision
        ...
        # mean precision evaluation
        eval_mean_precision = SGMeanPrecision(result_dict, num_rel_category, dataset.ind_to_predicates, print_detail=True)
        eval_mean_precision.register_container(mode)
        evaluator['eval_mean_precision'] = eval_mean_precision

        # mean F1-score evaluation
        eval_mean_f1_score = SGMeanF1Score(result_dict, num_rel_category, dataset.ind_to_predicates, print_detail=True)
        eval_mean_f1_score.register_container(mode)
        evaluator['eval_mean_f1_score'] = eval_mean_f1_score

        # calibration error evaluation
        eval_calibration_error = SGCalibrationError(result_dict)
        eval_calibration_error.register_container(mode)
        evaluator['eval_calibration_error'] = eval_calibration_error

        # no graph constraint precision evaluation
        eval_nog_precision = SGNoGraphConstraintPrecision(result_dict)
        eval_nog_precision.register_container(mode)
        evaluator['eval_nog_precision'] = eval_nog_precision

        # TopK evaluation metrics (more lenient evaluation)
        # Use both predicate-level TopK [1, 3, 5] and relation-level TopK [20, 50, 100]
        eval_topk_precision = SGTopKPrecision(result_dict, k_values=[20, 50, 100])
        eval_topk_precision.register_container(mode)
        evaluator['eval_topk_precision'] = eval_topk_precision

        eval_topk_f1_score = SGTopKF1Score(result_dict, k_values=[20, 50, 100])
        eval_topk_f1_score.register_container(mode)
        evaluator['eval_topk_f1_score'] = eval_topk_f1_score

        eval_topk_relation_precision = SGTopKRelationPrecision(result_dict, k_values=[20, 50, 100])
        eval_topk_relation_precision.register_container(mode)
        evaluator['eval_topk_relation_precision'] = eval_topk_relation_precision

        eval_mean_topk_precision = SGMeanTopKPrecision(result_dict, num_rel_category, dataset.ind_to_predicates, k_values=[20, 50, 100], print_detail=True)
        eval_mean_topk_precision.register_container(mode)
        evaluator['eval_mean_topk_precision'] = eval_mean_topk_precision
        ...
        eval_mean_precision.calculate_mean_precision(mode)
        
        # calculate mean F1-score
        eval_mean_f1_score.calculate_mean_f1(mode)
        
        # calculate mean TopK precision
        eval_mean_topk_precision.calculate_mean_topk_precision(mode)
        ...
        # print new precision, F1, and calibration metrics
        result_str += eval_precision.generate_print_string(mode)
        result_str += eval_nog_precision.generate_print_string(mode)
        result_str += eval_f1_score.generate_print_string(mode)
        result_str += eval_mean_precision.generate_print_string(mode)
        result_str += eval_mean_f1_score.generate_print_string(mode)
        result_str += eval_calibration_error.generate_print_string(mode)
        
        # TopK evaluation results (showing higher scores due to lenient evaluation)
        result_str += eval_topk_precision.generate_print_string(mode)
        result_str += eval_topk_f1_score.generate_print_string(mode)
        result_str += eval_topk_relation_precision.generate_print_string(mode)
        result_str += eval_mean_topk_precision.generate_print_string(mode)
...
def evaluate_relation_of_one_image(groundtruth, prediction, global_container, evaluator):
    ...
    # Precision
    if evaluator.get("eval_precision") is not None:
        evaluator['eval_precision'].calculate_precision(global_container, local_container, mode)
    
    # No Graph Constraint Precision
    if evaluator.get("eval_nog_precision") is not None:
        evaluator['eval_nog_precision'].calculate_precision(global_container, local_container, mode)
    
    # F1-Score (calculated after precision and recall)
    if evaluator.get("eval_f1_score") is not None:
        evaluator['eval_f1_score'].calculate_f1_score(global_container, local_container, mode)
    
    # Mean Precision
    if evaluator.get("eval_mean_precision") is not None:
        evaluator['eval_mean_precision'].collect_mean_precision_items(global_container, local_container, mode)
    
    # Mean F1-Score
    if evaluator.get("eval_mean_f1_score") is not None:
        evaluator['eval_mean_f1_score'].collect_mean_f1_items(global_container, local_container, mode)
    
    # Calibration Error
    if evaluator.get("eval_calibration_error") is not None:
        evaluator['eval_calibration_error'].calculate_calibration_error(global_container, local_container, mode)

    # TopK evaluation metrics
    if evaluator.get("eval_topk_precision") is not None:
        evaluator['eval_topk_precision'].calculate_topk_precision(global_container, local_container, mode)
    
    if evaluator.get("eval_topk_f1_score") is not None:
        evaluator['eval_topk_f1_score'].calculate_topk_f1_score(global_container, local_container, mode)
    
    if evaluator.get("eval_topk_relation_precision") is not None:
        evaluator['eval_topk_relation_precision'].calculate_topk_relation_precision(global_container, local_container, mode)
    
    if evaluator.get("eval_mean_topk_precision") is not None:
        evaluator['eval_mean_topk_precision'].collect_mean_topk_precision_items(global_container, local_container, mode)
