...
from maskrcnn_benchmark.data.datasets.visual_genome import get_VG_statistics
...
class OIDataset(torch.utils.data.Dataset):
    ...
    def get_statistics(self):
        ...
        # fg_matrix, bg_matrix, rel_class_num_lst, obj_class_num_lst, rel_counter_init = get_VG_statistics(self,
        #                                                            must_overlap=True)

        
        fg_matrix, bg_matrix, rel_class_num_lst, obj_class_num_lst = get_OI_statistics(img_dir=self.img_dir, roidb_file=self.annotation_file, 
        dict_file=self.cate_info_file, must_overlap=True)
...
# (self, split, img_dir, ann_file, cate_info_file, transforms=None,
#                  num_im=-1, check_img_file=False, filter_duplicate_rels=True, flip_aug=False):

# (img_dir=self.img_dir, roidb_file=self.annotation_file, dict_file=self.cate_info_file,
#                                                 image_file=self.image_file, must_overlap=True)

def get_OI_statistics(img_dir, roidb_file, dict_file, must_overlap=True):

    train_data = OIDataset(split='train', img_dir=img_dir, ann_file=roidb_file, 
                          cate_info_file=dict_file, transforms=None,
                          num_im=-1, check_img_file=False, filter_duplicate_rels=True, flip_aug=False)
