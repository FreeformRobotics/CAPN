...
class VCTreeLSTMContext(nn.Module):
    ...
    def forward(self, x, proposals, rel_pair_idxs, logger=None, all_average=False, ctx_average=False):
        ...
        if self.use_relation_aware_gating:
            return obj_dists, obj_preds, edge_ctx, bi_preds, beta_relation_aware_gating
        else:
...
class VCTreeLSTMContext_split(nn.Module):
    """
    Modified from neural-motifs to encode contexts for each objects
    """

    def __init__(self, config, obj_classes, rel_classes, statistics, in_channels):
        super(VCTreeLSTMContext_split, self).__init__()
        self.cfg = config
        self.obj_classes = obj_classes
        self.rel_classes = rel_classes
        self.num_obj_classes = len(obj_classes)
        ...
        self.edge_ctx_rnn = nn.ModuleList(
            ...
            out_dim=self.hidden_dim,
            num_layer=self.nl_edge,
            dropout=self.dropout_rate if self.nl_edge > 1 else 0, ) for _ in range(self.num_experts)]
        )
    ...
    def separate_edge_ctx(self, num_objs, obj_feats, forest, index=0):
        """
        Object context and object classification.
        :param obj_feats: [num_obj, img_dim + object embedding0 dim]
        :return: edge_ctx: [num_obj, #feats] For later!
        ...
        edge_ctxs = []
        for feat, tree, num_obj in zip(inp_feats, forest, num_objs):
            edge_rep = self.edge_ctx_rnn[index](tree, feat, num_obj)
            edge_ctxs.append(edge_rep)
    ...
    def forward(self, x, proposals, rel_pair_idxs, logger=None, all_average=False, ctx_average=False):
        ...
        if self.num_experts > 1:
            for experts_index in range(self.num_experts):
                out = self.separate_edge_ctx(num_objs, obj_rel_rep, vc_forest, experts_index)
                outs.append(out)
