...
class LSTMContext(nn.Module):
    ...
    def __init__(self, config, obj_classes, rel_classes, in_channels):
        ...
        self.use_relation_aware_gating = self.cfg.MODEL.ROI_RELATION_HEAD.USE_RELATION_AWARE_GATING

        if self.use_relation_aware_gating:
            self.relation_aware_gating = Relation_aware_gating(in_fea_dim=4808, num_experts=self.num_experts)
    ...
    def forward(self, x, proposals, rel_pair_idxs, logger=None, all_average=False, ctx_average=False):
        ...
        if self.use_relation_aware_gating:
            beta_relation_aware_gating = self.relation_aware_gating(obj_rel_rep)
        ...
        if self.use_relation_aware_gating:
            return obj_dists, obj_preds, edge_ctx, beta_relation_aware_gating, None
        else:
...
class LSTMContext_split(nn.Module):
    """
    Modified from neural-motifs to encode contexts for each objects
    """

    def __init__(self, config, obj_classes, rel_classes, in_channels):
        super(LSTMContext_split, self).__init__()
        self.cfg = config
        self.obj_classes = obj_classes
        self.rel_classes = rel_classes
        self.num_obj_classes = len(obj_classes)
        ...
        if self.num_experts > 1:
            self.edge_ctx_rnn = nn.ModuleList(
                [torch.nn.LSTM(
                input_size=self.embed_dim + self.hidden_dim + self.obj_dim,
                hidden_size=self.hidden_dim,
                num_layers=self.nl_edge,
                dropout=self.dropout_rate if self.nl_edge > 1 else 0,
                bidirectional=True) for _ in range(self.num_experts)]
            )
            for i in range(self.num_experts):
                for name, param in self.edge_ctx_rnn[i].named_parameters():
                    if name.startswith("weight"):
                        nn.init.xavier_normal_(param)
                    else:
                        nn.init.zeros_(param)

            print('self.edge_ctx_rnn:', self.edge_ctx_rnn)
            self.lin_edge_h = nn.ModuleList(
                [nn.Linear(self.hidden_dim * 2, self.hidden_dim) for _ in range(self.num_experts)]
            )
            for i in range(self.num_experts):
                for name, param in self.lin_edge_h[i].named_parameters():
                    if name.startswith("weight"):
                        nn.init.xavier_normal_(param)
                    else:
                        nn.init.zeros_(param)
            print('self.lin_edge_h:', self.lin_edge_h)
    ...
    def separate_edge_ctx(self, inp_feats, perm, inv_perm, ls_transposed, index=0):
        """
        Object context and object classification.
        :param obj_feats: [num_obj, img_dim + object embedding0 dim]
        :return: edge_ctx: [num_obj, #feats] For later!
        :index: to choose expert number
        """
        edge_input_packed = PackedSequence(inp_feats[perm], ls_transposed)
        if self.num_experts > 1:
            edge_reps = self.edge_ctx_rnn[index](edge_input_packed)[0][0]
            edge_reps = self.lin_edge_h[index](edge_reps)  # map to hidden_dim

        else:
            edge_reps = self.edge_ctx_rnn(edge_input_packed)[0][0]
            # print('edge_reps:', edge_reps.shape)
            edge_reps = self.lin_edge_h(edge_reps)  # map to hidden_dim
            # print('edge_reps:', edge_reps.shape)
    ...
    def forward(self, x, proposals, rel_pair_idxs, logger=None, all_average=False, ctx_average=False):
        ...
        # edge level contextual feature, [x, 200]
        obj_embed2 = self.obj_embed2(obj_preds.long())
        ...
        if self.num_experts > 1:
            for experts_index in range(self.num_experts):
                out = self.separate_edge_ctx(obj_rel_rep, perm=perm, inv_perm=inv_perm, ls_transposed=ls_transposed,
                                           index=experts_index)
                outs.append(out)
            ...
            edge_ctx = self.separate_edge_ctx(obj_rel_rep, perm=perm, inv_perm=inv_perm, ls_transposed=ls_transposed)
