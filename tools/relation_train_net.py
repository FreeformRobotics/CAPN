...
def train(cfg, local_rank, distributed, logger):
    ...
    for iteration, (images, targets, _) in enumerate(train_data_loader, start_iter):
        ...
        # print('images.shape:', images.image_sizes)
        # print('images.tensors:', images.tensors)
        # print('targets:', len(targets))

        # for target in targets:
        #     print('target', target.bbox.shape)
        #     print('target.extra_fields',
        #           target.extra_fields['labels'].shape,
        #           target.extra_fields['attributes'].shape,
        #           target.extra_fields['relation'].shape)
        #
        #     print('target.triplet_extra_fields', target.triplet_extra_fields)
        ...
        # save the checkpoints at every 2000 iterations
        if iteration % checkpoint_period == 0:
            # sgdet model
            if cfg.MODEL.ROI_RELATION_HEAD.USE_GT_BOX is False and cfg.MODEL.ROI_RELATION_HEAD.USE_GT_OBJECT_LABEL is False:
                if iteration >= 2000:
                    # motifs
                    if cfg.MODEL.ROI_RELATION_HEAD.PREDICTOR == 'MotifPredictor_CAME':
        ...
        if cfg.SOLVER.SCHEDULE.TYPE == "WarmupReduceLROnPlateau":
            ...
            if scheduler.stage_count >= cfg.SOLVER.SCHEDULE.MAX_DECAY_STEP and iteration >= 24000:
...
def parser_argument(cfg):
    ...
    parser.add_argument(
        "--loss_option",
        default='cross_entropy',          # CROSS_ENTROPY_LOSS, LABEL_SMOOTHING_LOSS, FOCAL_LOSS, LDAM_LOSS,  RIDE_LOSS
        help="choose which loss for training",
        type=str,
    )
    parser.add_argument(
        "--num_experts",
...
def main():
    ...
    cfg.MODEL.ROI_RELATION_HEAD.LOSS_OPTION = args.loss_option
    ...
    results = get_dataset_statistics(cfg)

    for rel in results['rel_class_num_lst']:
        cfg.MODEL.REL_CLASS_NUM_LST.append(rel)
    for obj in results['obj_class_num_lst']:
        cfg.MODEL.OBJ_CLASS_NUM_LST.append(obj)

    print('cfg.REL_CLASS_NUM_LST:', cfg.MODEL.REL_CLASS_NUM_LST)
    print('cfg.OBJ_CLASS_NUM_LST:', cfg.MODEL.OBJ_CLASS_NUM_LST)
